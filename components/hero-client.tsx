...
import { formatRelativeDate, parseSideBets } from '@/utils/formatters'
...
function HeroCopy({ siteSettings = null, consultingTiers = [] }: { siteSettings?: any, consultingTiers?: any[] }) {
  ...
  const yearsShipping = year - 2015

  const sideBets = parseSideBets(siteSettings?.meta_side_bets)
  ...
  return (
    <>
      ...
      <div className="hero-meta">
        <span><b>{yearsShipping}+</b> years shipping</span>
        <span><b>{total.toLocaleString()}</b> GitHub contributions / year</span>
        <span>
          {sideBets.number ? <b>{sideBets.number}</b> : null}
          {sideBets.text}
        </span>
...
export function HeroClient({ siteSettings = null, consultingTiers = [] }: { siteSettings?: any, consultingTiers?: any[] }) {
  const { t } = useTheme()
