...
export function useTweaks(defaults: any) {
  ...
  const setTweak = React.useCallback((keyOrEdits: any, val?: any) => {
    ...
    try {
      localStorage.setItem('isheraz.theme', edits.dark ? 'dark' : edits.dark === false ? 'light' : localStorage.getItem('isheraz.theme') || 'dark')
    } catch (e) {}
  }, [])
...
export function TweakRadio({ label, value, options, onChange }: any) {
  ...
  const segmentWidths: Record<number, number> = { 2: 16, 3: 10 }
  const fitsAsSegments = maxLen <= (segmentWidths[options.length] ?? 0)
